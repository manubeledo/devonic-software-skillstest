...
const Cards = ({progType, indexNum}: Props) => {
        ...
        <div className={styles.frameCards}>
            { 
                datos.map((card, index) => {
                    if(card.programType == progType && index<indexNum)
                    return(
                            <div className={styles.fullCard} key={index}>
                                <button onClick={() => { setShowModal(true), setCardInfo(card) }} className={styles.btn}>
                                <div className={styles.card}>
                                    <Image src={card.images['Poster Art'].url} alt="image" width={160} height={242}></Image>
                                </div>
                                </button>
